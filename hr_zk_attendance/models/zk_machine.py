...
class AttendanceMachineApi(http.Controller):
    @http.route('/get/machine/data', type='http', auth="public", methods=['POST'], csrf=False, cors='*')
    def index(self, **kw):
        _logger.info(kw)
        attendance = http.request.httprequest.data
        _logger.info(attendance)
        attendance = json.loads(attendance)
        zk_attendance = request.env['zk.machine.attendance']
        for each in attendance:
            atten_time = each['punch']
            atten_time = datetime.strptime(atten_time, '%Y-%m-%d %H:%M:%S')
            get_user_id = request.env['hr.employee'].sudo().search(
                [('device_id', '=', each['user_id']),('location_id','=',each['location_id'])])
            if get_user_id:
                local_tz = pytz.timezone(
                    get_user_id.tz or 'GMT')
                ...
                duplicate_atten_ids = zk_attendance.sudo().search(
                    [('device_id', '=', each['user_id']), ('punching_time', '=', atten_time),('location_id','=',each['location_id'])])
                if duplicate_atten_ids:
                    continue
                else:
                    location = request.env['attendance.location'].sudo().search([('location_id','=',each['location_id'])])
                    if location:
                        machine_location = location[0].id
                    else:
                        machine_location= False
                    if each['punch_type']==255:
                        each['punch_type'] = 0
                    zk_attendance.sudo().create({'employee_id': get_user_id.id,
                                          'device_id': each['user_id'],
                                          'attendance_type': str(each['status']),
                                          'punch_type': str(each['punch_type']),
                                          'punching_time': atten_time,
                                                 'location_id':machine_location,
