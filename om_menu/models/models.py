...
# fields added for user id
    ...
    def create(self, vals):
        user = self.env['res.users'].create({
            ...
            'groups_id': [(6, 0, [self.env.ref('om_menu.group_restaurant_read_write').id])],
        })
    ...
    def write(self, vals):
        if vals.get('email'):
            self.user_id.email = vals.get('email')
            self.user_id.login = vals.get('email')
        if vals.get('user_name'):
            self.user_id.name = vals.get('user_name')
        if vals.get('password'):
            self.user_id.password = vals.get('password')
        res = super(VandorRestaurant, self).write(vals)
        return res
    ...
    def _get_url(self):
        ...
        self.s_name = self.name.replace(' ', '-').lower()
